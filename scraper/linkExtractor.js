...
(async () => {
  try {
    ...
    if (urls.length > 0) {
      // Publish to Kafka
      console.log(`\nPublishing ${urls.length} URLs to Kafka topic '${KAFKA_TOPIC}'...`);
      const messages = urls.map(url => ({
        key: url.split('/video/')[1], // Use video ID as key
      ...
      console.log(`✓ Published ${urls.length} messages to Kafka`);
